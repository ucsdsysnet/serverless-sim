...
# atc_subset_of_apps <-
#     as_tibble(
#       read.csv("../azurefunction-dataset/list_of_atc_subset.csv",
#                header=FALSE,
#                col.names = c(
#                  "SubscriptionId",
#                  "AppName"
...
release_invocations_per_function <- 
  as_tibble(
    read.csv("../azurefunction-dataset/release_invocations_per_functions.csv",
      header = FALSE,
      col.names = c(
        "HashApp",
        "HashFunc",
...
release_invocations_per_function <- release_invocations_per_function %>%
        inner_join(list_of_release_apps, by = c("HashApp"))
...
fdurations <- 
    as_tibble(read.csv("../azurefunction-dataset/function_durations_percentiles.anon.d01.csv"), 
    header=TRUE
)
...
fdurations_fixmin <- fdurations %>% 
    select(c(HashOwner, HashApp, Average,Minimum,Maximum)) %>% 
    na.exclude() %>% 
...
fmem <- 
    as_tibble(
        read.csv("../azurefunction-dataset/app_memory_percentiles.anon.d01.csv", 
                 header=TRUE)
        )
fmem <- fmem %>% filter(AverageAllocatedMb_pct1 > 0)
...
ggplot() + 
    ...
    stat_ecdf(data=fmem_filtered,
              aes(x=AverageAllocatedMb, color = "Average", linetype = "Released")) +
    stat_ecdf(data=fmem_filtered,
              aes(x=AverageAllocatedMb_pct1, color = "1st Percentile", linetype = "Released")) +
    stat_ecdf(data=fmem_filtered,
              aes(x=AverageAllocatedMb_pct100, color = "Maximum", linetype = "Released")) + 
    scale_x_log10() +
    scale_y_continuous(breaks=c(0,.1,.25,.5,.75,.9,1)) + 
